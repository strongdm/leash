...
func newPolicyAPI(mgr *policy.Manager, policyPath string, broadcaster policyBroadcaster, mitmProxy *proxy.MITMProxy, wsHub *websockethub.WebSocketHub) *policyAPI {
	...
	if api.wsHub == nil {
		if h, ok := broadcaster.(*websockethub.WebSocketHub); ok {
			api.wsHub = h
		}
	}
...
func (api *policyAPI) handlePost(w http.ResponseWriter, r *http.Request) {
	...
	// Detect conflicts within the submitted Cedar set before compiling/applying
	if parser := transpiler.NewCedarParser(); parser != nil {
		if set, perr := parser.ParseFromString(cedar); perr == nil && set != nil {
			for i := range set.Policies {
				for j := i + 1; j < len(set.Policies); j++ {
					if policiesConflict(set.Policies[i], set.Policies[j]) {
						msg := fmt.Sprintf("%s conflicts with: %s", humanizedWithEffect(set.Policies[i]), humanizedWithEffect(set.Policies[j]))
	...
	// IR-level conflict check (same match with opposite effects)
	if msg := detectIRConflicts(compilation.Policies); msg != "" {
		writeJSON(w, http.StatusConflict, map[string]any{"error": msg})
		return
	}
...
func (api *policyAPI) handlePatch(w http.ResponseWriter, r *http.Request) {
	...
	// Pre-parse existing policies for conflict checks
	existingParser := transpiler.NewCedarParser()
	var existingSet *transpiler.CedarPolicySet
	if es, e := existingParser.ParseFromString(strings.Join(currentStatements, "\n\n")); e == nil {
		existingSet = es
	}
	...
	for i := len(newStatements) - 1; i >= 0; i-- {
		...
		// Conflict check against existing (and earlier accepted) policies
		if existingSet != nil {
			if newSet, err := existingParser.ParseFromString(stmt); err == nil && newSet != nil && len(newSet.Policies) > 0 {
				newPol := newSet.Policies[0]
				for _, ep := range existingSet.Policies {
					if policiesConflict(newPol, ep) {
						msg := fmt.Sprintf("%s conflicts with: %s", humanizedWithEffect(newPol), humanizedWithEffect(ep))
						writeJSON(w, http.StatusConflict, map[string]any{"error": msg})
						return
					}
				}
		...
		// Update existingSet incrementally so subsequent new statements are checked too
		if existingSet != nil {
			if more, err := existingParser.ParseFromString(stmt); err == nil && more != nil {
				existingSet.Policies = append(existingSet.Policies, more.Policies...)
			}
		}
	}
	...
	// Cedar-level conflict detection across the whole set (AST-based)
	if parser := transpiler.NewCedarParser(); parser != nil {
		if setAll, err := parser.ParseFromString(updated); err == nil && setAll != nil {
			if msg := findCedarConflict(setAll.Policies); msg != "" {
				writeJSON(w, http.StatusConflict, map[string]any{"error": msg})
				return
			}
		}
...
func (api *policyAPI) handlePersistPolicies(w http.ResponseWriter, r *http.Request) {
	...
	// Reject obvious logical conflicts (Cedar-level, AST-based) before any compile/persist.
	if repSet, err := transpiler.NewCedarParser().ParseFromString(cedar); err == nil && repSet != nil {
		if msg := findCedarConflict(repSet.Policies); msg != "" {
			writeJSON(w, http.StatusConflict, map[string]any{"error": msg})
			return
		}
	}
...
func (api *policyAPI) buildCompletionHints(id completionIDHints) autocomplete.Hints {
	...
	// Client-provided hints come last so runtime data retains priority.
...
// handlePanicReset installs a permissive policy in both runtime and file layers,
// persists Cedar to the configured policy file and switches to Enforce mode.
// (Panic Reset removed) — previously installed a permissive policy across
// runtime and file layers. This path was confusing; use /api/policies/permit-all
// for a runtime-only permissive mode, or POST /api/policies/persist with Cedar
// you explicitly author for file-layer policy changes.
...
func (api *policyAPI) currentCedarSnapshot() string {
	...
	if mode == "permit-all" {
		if trimmed := strings.TrimSpace(prev); trimmed != "" {
			return trimmed
		}
		if strings.TrimSpace(cedar) == "" {
			if cf, err := api.loadCanonicalCedar(); err == nil {
...
// detectIRConflicts inspects the compiled IR (PolicySet) for exact-match allow/deny pairs.
// Returns an error message suitable for user display when a conflict is found, otherwise empty string.
func detectIRConflicts(ps *lsm.PolicySet) string {
	if ps == nil {
		return ""
	}
	// Connect: conflict if same host/ip/port tuple appears with both allow and deny
	type seen struct{ allow, deny bool }
	seenMap := make(map[string]*seen)
	for _, r := range ps.Connect {
		key := "connect|"
		if r.HostnameLen > 0 {
			host := string(bytes.TrimRight(r.Hostname[:], "\x00"))
			key += "host=" + strings.ToLower(strings.TrimSpace(host))
		} else if r.DestIP != 0 {
			key += fmt.Sprintf("ip=%s", formatIPv4(r.DestIP))
		}
		if r.DestPort != 0 {
			key += fmt.Sprintf("|port=%d", r.DestPort)
		}
		if key == "connect|" {
			// wildcard *: don't consider this a direct conflict surface (deny *.com vs allow *) is not equal scope
			continue
		}
		entry := seenMap[key]
		if entry == nil {
	...
	for k, v := range seenMap {
		if v.allow && v.deny {
			// Build human-friendly target
			target := ""
			if strings.Contains(k, "host=") {
				parts := strings.Split(k, "host=")
				hostPart := parts[1]
				if idx := strings.Index(hostPart, "|port="); idx >= 0 {
					target = hostPart[:idx] + ":" + strings.TrimPrefix(hostPart[idx:], "|port=")
				} else {
					target = hostPart
				}
			} else if strings.Contains(k, "ip=") {
				parts := strings.Split(k, "ip=")
				ipPart := parts[1]
				if idx := strings.Index(ipPart, "|port="); idx >= 0 {
					target = ipPart[:idx] + ":" + strings.TrimPrefix(ipPart[idx:], "|port=")
				} else {
					target = ipPart
				}
			}
			...
			return fmt.Sprintf("Allow conflicts with: Deny Connect %s", target)
		}
	}
	// Exec/Open: treat identical path rule with opposite action as conflict
	pathSeen := make(map[string]*seen)
	recordPath := func(op string, r lsm.PolicyRule) {
		path := string(bytes.TrimRight(r.Path[:r.PathLen], "\x00"))
	...
	for k, v := range pathSeen {
		if v.allow && v.deny {
			...
			return fmt.Sprintf("Allow conflicts with: Deny %s %s", opLabel, path)
		}
	}
	return ""
}
...
func (api *policyAPI) addPolicyStatement(newCedar string) (map[string]any, int, error) {
	...
	if existing != "" {
		...
		if existingSet, err := existingParser.ParseFromString(existing); err == nil {
			// Conflict detection: same scope, opposite effect
			for _, existingPolicy := range existingSet.Policies {
				if policiesConflict(newPolicy, existingPolicy) {
					msg := fmt.Sprintf("%s conflicts with: %s", humanizedWithEffect(newPolicy), humanizedWithEffect(existingPolicy))
	...
	// IR-level conflict detection to catch exact-match allow/deny pairs
	if msg := detectIRConflicts(lsmRules); msg != "" {
		return nil, http.StatusConflict, errors.New(msg)
	}
...
func buildCedarFromActionRequest(req addPolicyFromActionRequest) (string, error) {
	...
	switch actType {
	case "file/open":
		// Default to generic open; directory vs file is expressed by the resource type.
		entity, _ := buildFileEntityFromName(name)
		return buildHeadEqualityPolicy(effect, `Action::"FileOpen"`, entity), nil
	case "file/write":
		// Treat writes as read-write opens in canonical Cedar.
		entity, _ := buildFileEntityFromName(name)
...
func humanizeAction(action string) string {
	...
	// No schema or legacy forms supported in policies; input is expected to be Action::PascalCase
	if i := strings.Index(lower, "action::"); i >= 0 {
		trimmed = trimmed[i+len("action::"):]
		trimmed = strings.Trim(trimmed, "\"")
		lower = strings.ToLower(trimmed)
	}
	// No fallback: only PascalCase Actions are humanized below
	switch lower {
	case "fileopen":
		return "open files"
...
func humanizeResource(resourceType, resourceID string) string {
	...
	id := cleanCedarLiteral(resourceID)
...
func cleanCedarLiteral(value string) string {
...
func splitResourceLiteral(literal string) (string, string, bool) {
	...
	value := cleanCedarLiteral(parts[len(parts)-1])
...
// policiesConflict returns true when two policies target the same scope but have opposite effects.
// It compares principal, action, resource, and (when available) the parsed AST of conditions.
// Effect is intentionally NOT compared for equality; instead, conflicts require different effects.
func policiesConflict(p1, p2 transpiler.CedarPolicy) bool {
	// Effects must differ to be a conflict
	if p1.Effect == p2.Effect {
		return false
	}
	if !entityConstraintsEqual(p1.Principal, p2.Principal) {
	...
	// Compare extracted condition sets (structural, effect-agnostic)
	if !conditionsEqual(p1.Conditions, p2.Conditions) {
		return false
	}
	return true
}

// humanizedWithEffect builds a concise human string like "Allow Connect api.example.com" from a policy.
func humanizedWithEffect(p transpiler.CedarPolicy) string {
	effect := ""
	switch p.Effect {
	case transpiler.Permit:
		effect = effectLabel("permit")
	case transpiler.Forbid:
		effect = effectLabel("forbid")
	default:
		effect = effectLabel("")
	}
	desc := describePolicy(p)
	if strings.TrimSpace(desc) == "" {
		return strings.TrimSpace(effect)
	}
	return strings.TrimSpace(effect + " " + desc)
}

func conditionsEqual(a, b []transpiler.Condition) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
		if a[i].Type != b[i].Type || a[i].Kind != b[i].Kind {
			return false
		}
		if a[i].ContextKey != b[i].ContextKey || a[i].ContextValue != b[i].ContextValue {
			return false
		}
		if len(a[i].ResourceSet) != len(b[i].ResourceSet) {
			return false
		}
		for j := range a[i].ResourceSet {
			if a[i].ResourceSet[j] != b[i].ResourceSet[j] {
				return false
			}
		}
		// Expression is a fallback string; compare if both non-empty
		if strings.TrimSpace(a[i].Expression) != strings.TrimSpace(b[i].Expression) {
			// If both are empty, treat as equal; otherwise mismatch
			if !(strings.TrimSpace(a[i].Expression) == "" && strings.TrimSpace(b[i].Expression) == "") {
				return false
			}
		}
...
// findCedarConflict scans a slice of Cedar policies and returns a single conflict message if any
// two policies target the same scope with opposite effects. Returns empty string if none found.
func findCedarConflict(policies []transpiler.CedarPolicy) string {
	n := len(policies)
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			if policiesConflict(policies[i], policies[j]) {
				// prefer to show the existing policy as the conflicting one; policies[i] is earlier in the set
				var msg string
				if policies[i].Effect == transpiler.Permit {
					// i is Allow, j is Deny
					msg = fmt.Sprintf("%s conflicts with: %s", humanizedWithEffect(policies[i]), humanizedWithEffect(policies[j]))
				} else {
					// i is Deny, j is Allow
					msg = fmt.Sprintf("%s conflicts with: %s", humanizedWithEffect(policies[i]), humanizedWithEffect(policies[j]))
				}
				return msg
			}
